...
class VRMCore {
                    ...
                    materials.forEach(mat => {
                        ...
                        if (mat.version === 'mtoon' || mat.isMToonMaterial || (mat.userData && mat.userData.vrmMaterialProperties)) {
                            ...
                            if (mat.shadingShiftFactor !== undefined) {
                                mat.shadingShiftFactor = -0.1; // 适度增加阴影覆盖，减少面部死白高亮
                            }
                            ...
                            // 4. 边缘光（Rim Light）- 完全压低，避免光晕导致的显白
                            if (mat.rimLightingMixFactor !== undefined) {
                                mat.rimLightingMixFactor = 0.05;
                            }

                            // 5. 环境光遮蔽/自发光压制
                            if (mat.giEqualizationFactor !== undefined) {
                                mat.giEqualizationFactor = 0.1; // 降低全局环境光均匀化，增加阴影层次感
                            }
    ...
    async init(canvasId, containerId, lightingConfig = null) {
        ...
        this.manager.camera.position.set(0.448, 1.415, -2.579); // 更新为用户定义的默认位置
        this.manager.camera.lookAt(0.064, 0.928, 0.230);    // 更新为用户定义的默认观察目标
        ...
        // --- 核心优化点：支持视网膜高分屏 ---
        this.manager.renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
        this.manager.renderer.setSize(width, height);

        if (typeof this.applyPerformanceSettings === 'function') {
            this.applyPerformanceSettings();
        }

        // 禁用阴影贴图节省 GPU 开销（VRoid Hub 风格无实时阴影）
        ...
        // 恢复为默认的 NoToneMapping，因为二次元 MToon 材质不需要复杂的电影级曲线映射，
        // 复杂的 ToneMapping 反而会使得颜色变暗发灰。LobeVidol 也是用默认的 NoToneMapping。
        ...
        // 不使用 OrbitControls，由 VRMInteraction 统一接管
        this.manager.controls = null;
        ...
        // 进一步降低方向光，增加对比度
        // 方向光
        const directionalLight = new THREE.DirectionalLight(0xffffff, 0.8); // 调低至 0.8
        directionalLight.position.set(1, 1, 1).normalize();
        directionalLight.castShadow = false;
        this.manager.scene.add(directionalLight);
        this.manager.mainLight = directionalLight;

        // 环境光
        const ambientLight = new THREE.AmbientLight(0xffffff, 0.4); // 调低至 0.4
        this.manager.scene.add(ambientLight);

        // 添加网格地面 (可选，为了视觉参照)
        const gridHelper = new THREE.GridHelper(50, 100, 0xaaaaaa, 0xaaaaaa);
        this.manager.scene.add(gridHelper);
    ...
    async loadModel(modelUrl, options = {}) {
        ...
        try {
            // 从 window 获取 GLTFLoader 和 VRMLoaderPlugin
            const GLTFLoader = window.GLTFLoader;
            const VRMLoaderPlugin = window.THREE_VRM?.VRMLoaderPlugin;

            if (!GLTFLoader) throw new Error('GLTFLoader 未挂载到 window。');
            if (!VRMLoaderPlugin) throw new Error('VRMLoaderPlugin 未挂载到 window.THREE_VRM。');
            ...
            // 性能优化：剔除不需要的顶点和骨骼，防止高面数模型造成严重卡顿（特别是从 VRoid Hub 获取的模型）
            if (window.THREE_VRM?.VRMUtils) {
                try {
                    window.THREE_VRM.VRMUtils.removeUnnecessaryVertices(gltf.scene);
                    window.THREE_VRM.VRMUtils.removeUnnecessaryJoints(gltf.scene);
                    console.log('[VRM Core] 成功应用 VRMUtils 性能优化 (移除多余顶点与关节)');
                } catch (optError) {
                    console.warn('[VRM Core] 应用 VRMUtils 性能优化失败:', optError);
                }
            }
            ...
            // 禁用错误的视锥剪裁（注：蒙皮网格的边界盒不准确，不禁用会导致部分身体展示时移出视锥就消失）
            // lobe-vidol 采用相同策略：Disable frustum culling
            ...
            try {
                ...
                // 服务端现在返回的是最后一次保存的单个配置对象
                if (allPreferences && allPreferences.model_path) {
                    const prefPath = allPreferences.model_path;
                    let isMatch = false;

                    if (prefPath === modelUrl) isMatch = true;
                    else {
                        const normalizedPrefPath = normalizePath(prefPath);
                        if (normalizedPrefPath && normalizedPrefPath === normalizedModelUrl) isMatch = true;
                        else {
                            const prefFilename = getFilename(prefPath);
                            if (modelFilename && prefFilename && prefFilename === modelFilename) isMatch = true;
                        }
                    }

                    if (isMatch) {
                        preferences = allPreferences;
                        console.log('[VRM Core] 找到匹配的保存配置:', preferences);
                    }
                }
            } catch (error) {
            ...
            if (preferences) {
                ...
                // 无偶好设置时，使用默认回退位置
                vrm.scene.position.set(0.064, -0.102, 0);
            }
            ...
            // 【已禁用】模型加载后的自动保存 rotation 逻辑
            // 原因：每次切换新模型时，因为新模型没有保存过旋转，会自动覆盖偏好文件中的 model_path
            // 现在保存操作只通过 UI 上的「保存初始视角」按钮手动触发
            ...
            //     };
            //     const currentScale = {
            //         x: vrm.scene.scale.x, y: vrm.scene.scale.y, z: vrm.scene.scale.z
            //     };
            //     this.saveUserPreferences(modelUrl, currentPosition, currentScale, detectedRotation, null)
            //         .catch(err => console.error(`[VRM Core] 自动保存rotation时出错:`, err));
            // }
            ...
            if (this.manager.camera && this.manager.camera.fov) {
                ...
                if (camPosValid) {
                    ...
                    // 同步更新 OrbitControls 的 target，确保旋转中心点与 _cameraTarget 一致
                    if (this.manager.controls) {
                    ...
                    const targetScreenHeight = screenHeight * 0.7; // 再次提高填充比例，让模型更大
                    const fov = this.manager.camera.fov * (Math.PI / 180);
                    const distance = (scaledModelHeight / 2) / Math.tan(fov / 2) / targetScreenHeight * screenHeight;

                    const isMobileDevice = screenWidth <= 768;
                    // 降低相机高度系数：从 +0.1 改为 -0.15，相机会看得更低，从而让模型在屏幕上往上移
                    const cameraY = center.y - (isMobileDevice ? scaledModelHeight * 0.1 : scaledModelHeight * 0.55);
                    // VRM 模型正面朝 -Z 方向，相机必须在 Z 负方向才能看到正面
