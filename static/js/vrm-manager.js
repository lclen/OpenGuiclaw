...
if (!THREE) {
    console.error('[VRM Manager] THREE.js 未加载，3D 功能将不可用');
}

/**
 * 高级眼球与视线平滑跟踪器 (基于极坐标与微扫视系统)
 * 从 @pixiv/three-vrm 的 VRMLookAt 扩展而来，加入了基于 Math.exp 的自然阻尼缓冲
 * 以及 SACCADE (生物眼睛无意识微动)
 */
class VRMLookAtSmoother {
    constructor(vrmLookAt) {
        this._vrmLookAt = vrmLookAt;

        // Saccade 配置相较真实人类稍大，因为二次元眼睛大且需要更明显的“生动感”
        this.SACCADE_MIN_INTERVAL = 0.5;
        this.SACCADE_PROC = 0.05;
        this.SACCADE_RADIUS = 3.0; // 微动半径 (度)

        this.smoothFactor = 4.0;
        this.userLimitAngle = 90.0;
        this.userTarget = null;
        this.enableSaccade = true;

        this._saccadeYaw = 0.0;
        this._saccadePitch = 0.0;
        this._saccadeTimer = 0.0;

        this._yawDamped = 0.0;
        this._pitchDamped = 0.0;

        this._v3A = new window.THREE.Vector3();
    }

    update(delta) {
        if (!this._vrmLookAt || !this._vrmLookAt.autoUpdate) return;

        // 通过 `lookAt` 的内置计算得到目标的原本 Yaw/Pitch
        const orgTarget = this._vrmLookAt.target;

        // 目标为相机或 UserTarget
        if (this.userTarget) {
            this._vrmLookAt.lookAt(this.userTarget.getWorldPosition(this._v3A));
        } else if (orgTarget) {
            this._vrmLookAt.lookAt(orgTarget.getWorldPosition(this._v3A));
        }

        const yawTarget = this._vrmLookAt.yaw;
        const pitchTarget = this._vrmLookAt.pitch;

        // 指数平滑过度 (Math.exp)，消除帧率波动导致的卡顿，展现真实肌肉缓冲
        const k = 1.0 - Math.exp(-this.smoothFactor * delta);
        this._yawDamped += (yawTarget - this._yawDamped) * k;
        this._pitchDamped += (pitchTarget - this._pitchDamped) * k;

        let yawFrame = this._yawDamped;
        let pitchFrame = this._pitchDamped;

        if (this.enableSaccade) {
            if (this.SACCADE_MIN_INTERVAL < this._saccadeTimer && Math.random() < this.SACCADE_PROC) {
                // 生成极坐标系下的随机偏转 (度)
                this._saccadeYaw = (2.0 * Math.random() - 1.0) * this.SACCADE_RADIUS;
                this._saccadePitch = (2.0 * Math.random() - 1.0) * this.SACCADE_RADIUS;
                this._saccadeTimer = 0.0;
            }
        ...
        // 最终更新并跳过原内部 `lookAt` 应用 (改由直接传参)
        this._vrmLookAt.applier.applyYawPitch(yawFrame, pitchFrame);
        this._vrmLookAt._needsUpdate = false;

        // 恢复原始 target 防丢失
        this._vrmLookAt.target = orgTarget;
    }
}
...
class VRMManager {
    constructor() {
        ...
        // 预分配可复用的 Vector3，避免每帧 new THREE.Vector3() 触发 GC
        this._lightOffset = null; // 在 THREE 就绪后初始化
    ...
    _initMouseLookAtTracking() {
        ...
        if (!this._mouseMoveHandler) {
            this._lastLookAtMouseX = null;
            this._lastLookAtMouseY = null;
            this._lookAtPendingFrame = null;

            const performLookAtCalculation = () => {
                this._lookAtPendingFrame = null;
                if (this._lastLookAtMouseX !== null && this._lastLookAtMouseY !== null) {
                    this._setLookAtTargetByMouse(this._lastLookAtMouseX, this._lastLookAtMouseY);
                }
            };
            ...
            this._mouseMoveHandler = (event) => {
                ...
                if (this._lookAtPendingFrame === null) {
                    this._lookAtPendingFrame = requestAnimationFrame(performLookAtCalculation);
                }
            };
    ...
    startAnimateLoop() {
        ...
        const animateLoop = () => {
            ...
            // 移除人工帧率限制，让 requestAnimationFrame 自动同步显示器刷新率（如 144hz），实现极致流畅
            ...
            if (this.currentModel && this.currentModel.vrm) {
                // LobeVidol 风格：主光是固定在 (1, 1, 1) 的全局环境光，不跟随相机移动，以保持二次元 MToon 材质对光影角度的预期。
                ...
                if (this.currentModel.vrm.lookAt) {
                    ...
                    if (this._cursorFollow && this._cursorFollow.eyesTarget) {
                        targetPoint = this._cursorFollow.eyesTarget.position;
                    } else if (this._lookAtTarget && this._lookAtDesiredPoint) {
                        // 在这层不再做平滑，平滑统一交给 VRMLookAtSmoother
                        this._lookAtTarget.position.copy(this._lookAtDesiredPoint);
                        targetPoint = this._lookAtTarget.position;
                    }

                    if (this._lookAtSmoother) {
                        // 使用新的平滑器接管目标
                        if (targetPoint) {
                            if (!this._lookAtSmoother.userTarget) {
                                this._lookAtSmoother.userTarget = new window.THREE.Object3D();
                                if (this.scene) this.scene.add(this._lookAtSmoother.userTarget);
                            }
                            this._lookAtSmoother.userTarget.position.copy(targetPoint);
                        } else {
                            if (this._lookAtSmoother.userTarget) {
                                this._lookAtSmoother.userTarget = null;
                            }
                        }
                        this._lookAtSmoother.update(delta);
                    } else {
                        // 回退逻辑 (兜底)
                        this.currentModel.vrm.lookAt.target = targetPoint ? this._lookAtTarget : this.camera;
                    }
                }
                ...
                // 5. VRM 核心更新（LookAt / NodeConstraints / MToon / SpringBone）
                // 性能优化: 采用 Fixed Time Step (固定步长累加器) 的物理抗锯齿方案，取代之前的暴力跳帧。
                // 这能确保即使在掉帧严重的情况下，头发裙摆的物理也不会“爆炸”或“瞬移”。
                const quality = window.renderQuality || (this.core && this.core.performanceMode ? this.core.performanceMode : 'medium');
                const vrm = this.currentModel.vrm;

                // 初始化物理累加器
                if (!this._physicsAccumulator) this._physicsAccumulator = 0;

                // 限制单帧最大 delta 防止长时间挂起后切回爆裂
                const safeDelta = Math.min(delta, 0.1);

                // 根据画质设定物理计算频率 (Hz)
                // high: 60Hz (0.016s), medium: 45Hz (0.022s), low: 30Hz (0.033s)
                let physicsTimeStep = 0.016;
                if (quality === 'medium') physicsTimeStep = 0.022;
                if (quality === 'low') physicsTimeStep = 0.033;

                this._physicsAccumulator += safeDelta;

                // 非 SpringBone 组件强制每帧更新（不受画质节流影响）
                // 先临时剥离 springBoneManager
                const originalSpringBone = vrm.springBoneManager;
                vrm.springBoneManager = null;
                vrm.update(safeDelta);
                vrm.springBoneManager = originalSpringBone;

                // SpringBone 固定频率更新
                if (this.enablePhysics && originalSpringBone) {
                    // 当累积的时间大于设定的步长时，执行一到多次物理更新
                    while (this._physicsAccumulator >= physicsTimeStep) {
                        originalSpringBone.update(physicsTimeStep);
                        this._physicsAccumulator -= physicsTimeStep;
                    }
                    // 注：LobeVidol 般极致的做法还会利用残余的 _physicsAccumulator 计算 slerp 插值，
                    // 但对于 three-vrm 的 SpringBoneManager 来说，目前只要保持 update 步长稳定，
                    // 即可消除 90% 的突跳感。
                }

                // 恢复 SpringBone (已在上方完成)
                vrm.springBoneManager = originalSpringBone;
    ...
    async loadModel(modelUrl, options = {}) {
        ...
        if (this.renderer && this.renderer.domElement) {
            this.renderer.domElement.style.transition = 'opacity 0.4s ease-out';
            this.renderer.domElement.style.opacity = '0';

            // 如果已有模型，等待淡出完成，避免在淡出中途突变卸载
        ...
        // 初始化高级平滑眼球跟踪器
        if (result && result.vrm && result.vrm.lookAt) {
            this._lookAtSmoother = new VRMLookAtSmoother(result.vrm.lookAt);
            // vrm.lookAt 默认 autoUpdate 是 false，需要打开
            result.vrm.lookAt.autoUpdate = true;
        } else {
            this._lookAtSmoother = null;
        }
...
// 等待被其他文件调用的 VRMLookAtSmoother (已内联到顶部)
